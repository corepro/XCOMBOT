...
TOUTIAO_LOGIN_URL = "https://sso.toutiao.com"
...
def _has_login_prompt(page: Page) -> bool:
    ...
    # 如果仍在登录页面，视为未登录
    try:
        if "sso.toutiao.com" in (page.url or "") or "login" in (page.url or ""):
            return True
    except Exception:
        pass
    return False
...
def _has_auth_cookie(page: Page) -> bool:
    try:
        cookies = page.context.cookies([TOUTIAO_HOME])
        names = {c.get("name") for c in cookies}
        return "sessionid" in names or "uid_tt" in names or "sid_tt" in names
    except Exception:
        return False
...
def is_logged_in_toutiao(page: Page) -> bool:
    return get_logged_in_username(page) is not None


def ensure_login_toutiao(page: Page, timeout_sec: int = 300, on_logged_in=None) -> bool:
    """确保今日头条登录（旧接口，推荐使用 login_flow_toutiao(bm) 或 ensure_login_toutiao_with_save(bm)）"""
    page.goto(TOUTIAO_HOME)
    page.wait_for_load_state("domcontentloaded")

    if is_logged_in_toutiao(page):
        logger.info("Toutiao: 检测到已登录。")
        if on_logged_in:
            try:
                on_logged_in()
            except Exception:
    ...
    logger.info("Toutiao: 未检测到登录状态，请在打开的页面完成登录（扫码/密码），最多等待{}秒…", timeout_sec)
    ...
    logger.warning("Toutiao: 登录等待超时；请确认是否已登录。")
    return False


def ensure_login_toutiao_with_save(bm: BrowserManager, timeout_sec: int = 300) -> bool:
    """确保今日头条登录并自动保存Cookie状态"""
    page = bm.page
...
def login_flow_toutiao(bm: BrowserManager, timeout_sec: int = 300) -> None:
    """打开今日头条登录页并等待登录成功"""
    page = bm.page
    assert page is not None
    logger.info("打开 Toutiao 登录页: {}", TOUTIAO_LOGIN_URL)
    ...
    while time.time() < deadline:
        ...
        if ("toutiao.com" in cur and "sso.toutiao.com" not in cur and "login" not in cur) or _has_logged_in_ui(page) or _has_auth_cookie(page):
            logger.info("Toutiao: 登录成功，当前URL={}。保存会话…", cur)
            bm.save_storage()
            return
        if time.time() - last_log > 10:
            ...
            last_log = time.time()
        time.sleep(1.0)
    raise TimeoutError("Toutiao 登录等待超时")
...
def login(bm: BrowserManager, timeout_sec: int = 300) -> bool:
    """兼容性登录函数"""
    try:
        login_flow_toutiao(bm, timeout_sec)
        return True
    except Exception:
        return False
...
def check_login_status(page) -> bool:
    """兼容性登录状态检查函数"""
    return is_logged_in_toutiao(page)
